...
export default function AdminEmployeesScreen() {
  ...
  const handleToggle = (emp: any) => {
    Alert.alert(
      ...
      `Are you sure you want to ${emp.isActive
        ? 'deactivate' : 'activate'} ${emp.name}?`,
      [
      ...
      <View style={styles.empActions}>
        ...
        <TouchableOpacity
          style={[styles.toggleBtn,
            { backgroundColor: item.isActive ? '#FFF0F0' : '#E8F8F2' }]}
          onPress={() => handleToggle(item)}
        >
          <Ionicons
            name={item.isActive ? 'close-circle' : 'checkmark-circle'}
            size={18}
            color={item.isActive ? colors.error : colors.primary}
          />
        </TouchableOpacity>
      </View>
    </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  empLeft: { flexDirection: 'row', alignItems: 'center',
